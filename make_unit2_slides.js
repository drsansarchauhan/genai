...
// ── Extra slides to reach 100 ──────────────────────────────────────────────

// Extra L09: Many-shot + Bootstrap
bulletSlide('Many-Shot & Bootstrap Techniques',[
  'Claude 200K context enables 50-500 demonstrations in a single prompt',
  'Many-shot quality approaches fine-tuning for narrow classification and extraction tasks',
  'Cost trade-off: 200 demos add ~40K tokens per call — significant at API scale',
  'Rule of thumb: below 8 examples = few-shot; above 20 = evaluate cost vs quality gain',
  'Bootstrap: no data? Ask Claude to generate 10 examples, then use them as demonstrations',
  'Self-generated examples work surprisingly well for rapid prototyping without a labelled dataset',
], 'L09 | Many-Shot', 24);

bulletSlide('Zero-Shot vs Few-Shot: Decision Guide',[
  'Use zero-shot when: task is common and well-defined, free-form output is acceptable',
  'Use zero-shot when: token budget is tight or you are in rapid prototyping phase',
  'Use few-shot when: task is novel or specialised, strict output schema required',
  'Use few-shot when: you have 2-20 quality labelled examples available',
  'Transition point: if zero-shot scores below your target after 3 iterations, switch to few-shot',
  'Meta-rule: demonstrate the output you want, not just describe it',
], 'L09 | Decision', 25);

// Extra L10: Practical CoT
bulletSlide('Practical Chain-of-Thought Patterns',[
  'Pattern 1 PREP: Problem restatement, Relevant facts, Execution, Plausibility check',
  'Pattern 2 Scratchpad: "Use a scratchpad in <thinking> tags before answering"',
  'Pattern 3 Stepwise: "Number each step. Show all arithmetic. State each assumption."',
  'Pattern 4 Hypothesis: "Form a hypothesis, test it, revise if wrong, then conclude"',
  'Pattern 5 Devil\'s advocate: "Argue both sides before giving your final view"',
  'Choose the pattern that matches the reasoning structure your task requires',
], 'L10 | Patterns', 37);

// Extra L11: Consistency
bulletSlide('Tone Consistency Across Multi-Turn Conversations',[
  'Problem: persona drift — Claude gradually shifts away from specified tone after 10+ turns',
  'Root cause: later turns overwrite earlier context; tone specs in system prompt fade',
  'Fix 1: Include 2 sample exchanges in the system prompt to show the target tone concretely',
  'Fix 2: Append short persona reminder to user messages: [Respond as Priya]',
  'Fix 3: List forbidden behaviors explicitly — Claude avoids negatives more reliably than hitting positives',
  'Fix 4: Use lower temperature (0.3-0.5) for persona-critical production applications',
], 'L11 | Consistency', 49);

// Extra L12: Advanced format
bulletSlide('Advanced Format Patterns in Production',[
  'Pattern 1 Conditional format: "If confidence < 0.7, include an uncertainty flag field"',
  'Pattern 2 Nested schemas: use XML for complex nested structures; JSON for flat data',
  'Pattern 3 Streaming output: when streaming, ensure each chunk is valid partial JSON',
  'Pattern 4 Format + reasoning: ask for JSON with a "reasoning" field to get auditable output',
  'Pattern 5 Table fallback: if JSON fails 3 retries, fall back to markdown table + parse manually',
  'Always instrument format compliance rate in production — target >99% before shipping',
], 'L12 | Advanced', 61);

// Extra L13: RAG
bulletSlide('Retrieval-Augmented Context Management',[
  'RAG = store context (documents, past turns, facts) in a vector database',
  'At query time: retrieve top-K most semantically relevant chunks and inject into context',
  'Advantage: unlimited external knowledge without filling the context window',
  'Tools: ChromaDB, Pinecone, Weaviate, pgvector, FAISS for vector storage',
  'Embedding model: text-embedding-3-small (OpenAI) or voyage-2 (Anthropic partner)',
  'Full RAG pipeline: chunk -> embed -> store -> retrieve -> inject -> generate',
], 'L13 | RAG', 73);

// Extra L14: Testing at scale
bulletSlide('Building a Prompt Test Suite',[
  'Minimum viable test suite: 20 cases — 10 typical, 5 edge cases, 5 adversarial inputs',
  'Coverage matrix: at least 2 examples per output class or schema variant',
  'Golden set: 5-10 cases with human-verified ideal responses for regression testing',
  'Regression test: run full suite on every prompt change before deploying',
  'CI integration: add prompt test suite to your CI pipeline — fail build if accuracy drops',
  'Benchmark cadence: re-evaluate monthly as model versions update (Claude versions change)',
], 'L14 | Test Suite', 85);

// Extra L15: Capstone summary x2
bulletSlide('Prompt Engineering Best Practices Checklist',[
  'Before writing: define success criteria and build a 20-case test suite first',
  'Structure: always include all 4 components — instruction, context, input, output format',
  'Format: show the exact template you want, seed the assistant turn when possible',
  'Reasoning: add "think step by step" for any task requiring multi-step logic',
  'Validation: Pydantic + retry loop for all structured output in production code',
  'Versioning: git-track every prompt with performance metadata and changelog',
], 'L15 | Best Practices', 95);

summarySlide('CSAI601 Unit 2 — Complete',[
  {icon:'💡',title:'Zero & Few-Shot',body:'Right demo strategy for any task. Bootstrap when no data available.'},
  {icon:'🧠',title:'Chain-of-Thought',body:'Step-by-step reasoning. Extended Thinking for hardest tasks.'},
  {icon:'🌟',title:'Persona & Tone',body:'Stable multi-turn characters with controlled communication rules.'},
  {icon:'{}',title:'Structured Output',body:'Schema-validated JSON/XML. Retry with error feedback. Pydantic.'},
  {icon:'🎀',title:'Context Mgmt',body:'Summarisation, chunking, hierarchical memory, RAG for long sessions.'},
  {icon:'📊',title:'Eval & Iteration',body:'Metrics, A/B testing, LLM-as-judge, version control. Treat prompts as code.'},
],'Unit 2 | Complete',100);

bulletSlide('Unit 2 — Key Concepts Review',[
  'Prompt anatomy: instruction + context + input data + output format — all 4 always',
  'Zero-shot for common tasks; few-shot when format or domain precision is required',
  'Chain-of-thought: "think step by step" is the single highest-impact zero-cost improvement',
  'Persona = identity + expertise + communication rules + constraints — specify all four layers',
  'Structured output: show the schema, seed the response, validate with Pydantic, retry on error',
  'Context management: summarise history, chunk documents, place critical info at top and bottom',
],'Unit 2 | Review',99);


bulletSlide('Looking Ahead — Unit 3 Preview',[
  'Unit 3: Building Applications with Claude API — connecting prompts to real systems',
  'Topics: API integration, tool use, function calling, streaming responses',
  'Unit 4: Multi-agent systems, memory architectures, and agentic workflows',
  'Unit 5: Responsible AI, safety, evaluation at scale, and deployment best practices',
  'Foundation: every Unit 3-5 concept builds directly on Unit 2 prompt engineering skills',
  'Next step: upload all files to GitHub and continue learning from the lecture notes',
],'Unit 2 | Next Up',100);
